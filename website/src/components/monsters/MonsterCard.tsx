...
export default function MonsterCard({ monster, typeMap, onClick }: Props) {
  const total = getStatTotal(monster.baseStats);
  const { baseColor, accentColor, shape } = monster.spriteConfig;
  const clip = shapeClips[shape] || shapeClips.amorphous;
  const primaryType = typeMap[monster.type[0]];

  return (
    <button
      onClick={onClick}
      role="listitem"
      aria-label={`${monster.name} - ${monster.type.map(t => typeMap[t]?.name).join('/')} 타입, 총 스탯 ${total}`}
      ...
      style={{ fontFamily: 'inherit', boxShadow: '3px 3px 0 rgba(0,0,0,0.6)' }}
    >
      ...
      <div class="relative">
        ...
        <div class="w-full aspect-square mb-2 flex items-center justify-center relative" style={{ background: 'rgba(0,0,0,0.15)' }}>
          ...
          <span class="absolute top-1 left-1 text-[9px] text-pk-muted/60">
            #{String(monster.id).padStart(3, '0')}
          </span>
        </div>

        {/* Name */}
        <p class="text-xs text-pk-text truncate mb-1.5">{monster.name}</p>
