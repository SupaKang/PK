...
export default function MonsterDetail({ monster, typeMap, skillMap, chain, onClose }: Props) {
  const primaryColor = typeMap[monster.type[0]]?.color ?? '#888';
    ...
    >
      ...
      >
        ...
        <div class="flex items-center justify-between p-5 border-b border-pk-border/30 sticky top-0 bg-pk-bg/95 backdrop-blur-sm z-10"
          ...
          <div>
            <span class="text-[10px] text-pk-muted tracking-wider">#{String(monster.id).padStart(3, '0')}</span>
            <h2 class="text-xl leading-tight" style={{ color: primaryColor }}>{monster.name}</h2>
        ...
        <div class="p-5 space-y-6">
          {/* Description */}
          <p class="text-sm text-pk-text-secondary leading-relaxed">{monster.description}</p>

          {/* Stats */}
          <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              <h3 class="text-sm text-pk-accent mb-3">기본 스탯</h3>
              <StatBars stats={monster.baseStats} color={primaryColor} />
            </div>
            <div class="flex items-center justify-center">
              <StatRadar stats={monster.baseStats} color={primaryColor} />
            </div>
          </div>
          ...
          <div class="grid grid-cols-3 gap-3 text-center">
            {[
              { label: '포획률', value: monster.catchRate },
              { label: '경험치', value: monster.expYield },
              { label: '성장률', value: monster.growthRate },
            ].map(info => (
          ...
          {chain.length > 1 && (
            <div>
              ...
              <EvolutionTree chain={chain} typeMap={typeMap} currentId={monster.id} />
            </div>
          )}
          ...
          <div>
            <h3 class="text-sm text-pk-accent mb-3">
              ...
              <span class="text-pk-muted text-[10px] ml-2">
                ({monster.learnset.length + (monster.uniqueSkill ? 1 : 0)})
              </span>
            </h3>
            <div class="overflow-x-auto">
              <table class="w-full text-xs" role="table">
                ...
                <tbody>
                  {[...monster.learnset, ...(monster.uniqueSkill ? [monster.uniqueSkill] : [])].map(ls => {
                    const s = skillMap[ls.skillId];
                    if (!s) return null;
                    const isUnique = monster.uniqueSkill?.skillId === ls.skillId;
                    return (
