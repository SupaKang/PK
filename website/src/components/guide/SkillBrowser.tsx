...
interface Props { skills: Skill[]; types: TypeInfo[]; }
...
export default function SkillBrowser({ skills, types }: Props) {
      ...
      <div class="flex flex-wrap gap-3 mb-5" role="search" aria-label="스킬 필터">
        <div>
          ...
          <input
            ...
            style={{ fontFamily: 'inherit', minWidth: '150px' }}
          />
        </div>
        ...
        <table class="w-full text-xs" role="table">
          ...
          <tbody>
            {paged.map(s => {
              ...
              return (
                <tr key={s.id} class="border-b border-pk-border/10 hover:bg-pk-surface/30 transition-colors">
                  <td class="py-1.5 px-2 text-pk-text">{s.name}</td>
