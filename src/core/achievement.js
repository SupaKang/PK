...
const ACHIEVEMENTS = [
  ...
  { id: 'gym_1', name: 'FIRST BADGE', desc: 'Defeat your first gym leader', tier: 'bronze', check: (g) => (g.storyEngine?.gymsDefeated?.length || 0) >= 1 },
  { id: 'gym_4', name: 'HALF BADGES', desc: 'Earn 4 gym badges', tier: 'silver', check: (g) => (g.storyEngine?.gymsDefeated?.length || 0) >= 4 },
  { id: 'gym_8', name: 'ALL BADGES', desc: 'Earn all 8 gym badges', tier: 'gold', check: (g) => (g.storyEngine?.gymsDefeated?.length || 0) >= 8 },
